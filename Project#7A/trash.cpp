...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Michael Smith" };
const char *GLUITITLE   = { "User Interface Window" };

// random parameters:	

/*
clang++ -Xpreprocessor -fopenmp -I/usr/local/include -L/usr/local/lib -lomp -framework GLUT -framework OpenGL -framework OpenCL -L./lib -lglui sample.cpp -o main
*/
...
const int NUM_PARTICLES = 1024*1024*1.5*1.5*1.5*1.5*1.5*1.5; // <---------------------------------------------------
const int LOCAL_SIZE    = 32;                               
const char *CL_FILE_NAME   = { "particles.cl" };
//const char *CL_FILE_NAME   = { "keaton09.cl" };
//const char *CL_FILE_NAME   = { "hart09.cl" };
//const char *CL_FILE_NAME   = { "arnold09.cl" };
//const char *CL_FILE_NAME   = { "connor09.cl" };
...
Display( )
{
	...
	{
		char str[128];
		sprintf( str, "Frame %8d -- %6.1f GigaParticles/Sec", Frame, (float)NUM_PARTICLES/ElapsedTime/1000000000. );

          // Get performance values in GigaParticle/sec
          //printf("%6.1f\n", (float)NUM_PARTICLES / ElapsedTime / 1000000000.); // <---------------------------------------------------
          // Get performance values in MegaParticle/sec
          printf("%6.1f\n", (float)NUM_PARTICLES / ElapsedTime / 1000000.); // <---------------------------------------------------
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		...
		glPopMatrix( );
          ...
          glutWireSphere(42., 100, 100);
          glPopMatrix();

          // Sphere3
          glColor3f(.3f, .3f, 1.);
          glPushMatrix();
          glTranslatef(-550., -400., -150.);
          glutWireSphere(350., 100, 100);
          glPopMatrix();
