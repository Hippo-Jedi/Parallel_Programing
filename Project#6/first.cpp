...
main( int argc, char *argv[ ] )
{
  ...
  char *FUNC1 = (char *)"ArrayMult";
  char *FUNC2 = (char *)"ArrayMultAdd";
  bool ifSum = false;
  char *funcName;
	FILE *fp;
#ifdef WIN32
  ...
  if(part == 2){
    dD = clEnqueueWriteBuffer( cmdQueue, dD, CL_FALSE, 0, dataSize, hD, 0, NULL, NULL );
    if( status != CL_SUCCESS )
  ...
  if (part == 2){
    status = clSetKernelArg( kernal, 3, sizeof(cl_mem), &dD );
    if (status != CL_SUCCESS)
